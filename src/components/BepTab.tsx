...
export default function BepTab({ calculatedProducts }: BepTabProps) {
  const [monthlyOverhead, setMonthlyOverhead] = useState(15000000); // 15jt default
  const [gajiKaryawan, setGajiKaryawan] = useState(8000000);
  const [sewaRuko, setSewaRuko] = useState(5000000);
  const [listrikGas, setListrikGas] = useState(2000000);

  // Totals
  const totalFixedCosts = [monthlyOverhead, gajiKaryawan, sewaRuko, listrikGas].reduce((a, b) => a + b, 0);

  // Calculate average margin and revenue per product
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-2xl shadow-xs border border-gray-100">
        ...
        <p className="text-xs text-gray-500 mt-1">Hitung berapa banyak produk yang harus terjual agar bisnis mencapai titik impas (BEP).</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-xs space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div>
              <label className="block text-[10px] uppercase font-bold text-gray-500 mb-1">Biaya Ops Lainnya (pemasaran, dll)</label>
            ...
            <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 text-xs space-y-1">
              <span className="font-bold text-blue-800 block">📊 Rata-rata Produk</span>
              <div className="flex justify-between"><span className="text-blue-600">Harga Jual/Porsi:</span><span className="font-mono font-bold">{formatCurrency(avgRevenuePerUnit)}</span></div>
              <div className="flex justify-between"><span className="text-blue-600">HPP/Porsi:</span><span className="font-mono font-bold">{formatCurrency(avgHppPerUnit)}</span></div>
              <div className="flex justify-between"><span className="text-blue-600">Laba/Porsi:</span><span className="font-mono font-bold text-emerald-700">{formatCurrency(avgProfitPerUnit)}</span></div>
                  ...
                  {volumeScenarios.map((s, i) => (
                    <tr key={i} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3 text-center">
                        {s.volume >= bepUnits ? (
                          <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full font-bold text-[9px]">✅ Untung</span>
                        ) : (
                          <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full font-bold text-[9px]">⚠️ Rugi</span>
                        )}
                      </td>
                    </tr>
                  ))}
