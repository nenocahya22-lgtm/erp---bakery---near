...
export const SellerDashboard: React.FC = () => {
  ...
  const handleSeedSampleProducts = async () => {
    try {
      setIsLoading(true);
      const prCol = collection(db, 'products');
      
      // Double check first if products collection is empty
      const currentSnap = await getDocs(prCol);
      if (currentSnap.size > 0) {
        triggerToast('Tolak Seeding', 'Koleksi produk sudah memiliki data belanja. Silakan kelola secara manual.');
        setIsLoading(false);
        return;
      }
      ...
      triggerToast('Seeding Selesai', 'Uji coba berhasil! 6 Produk premium Shopee-style ditambahkan.');
    } catch (e) {
      console.error(e);
      triggerToast('Gagal Seeding', 'Gagal memuat produk contoh.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-100 pb-5 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-extrabold text-[11px] px-3.5 py-2 rounded-md shadow-xs transition-colors flex items-center gap-1 leading-none uppercase"
            >
              <Database size={13} />
              <span>Muat Hub Produk Contoh</span>
            </button>
          </div>
        </div>
          ...
          <>
            ...
            {activeTab === 'products' && (
              <div className="space-y-6">
                ...
                {/* Directory table matching products */}
                {products.length === 0 ? (
                  <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-lg">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Klik "<strong>Muat Hub Produk Contoh</strong>" di bar atas untuk memuat menu hidangan contoh instan secara otomatis.
                    </p>
                  </div>
                ) : (
